# 🎨 Cloth Config GUI 및 ModMenu (MC 26.1.2)
...
| **GUI 라이브러리** | Cloth Config Fabric (`me.shedaniel.cloth:cloth-config-fabric:26.1.154`) |
| **ModMenu 엔트리포인트** | `net.instantgratification.magnet.config.ModMenuIntegration` |
| **GUI 스크린 헬퍼** | `net.instantgratification.magnet.config.ClothConfigScreenHelper` |
...
마인크래프트 26.1.2에서 **Magnet, Let me get that!** 은 **Cloth Config Fabric** 및 **ModMenu** 와 연동되어 인게임 그래픽 설정 화면을 제공합니다.
...
> 이미 플레이 중인 기존 월드의 설정을 변경하려면 `/gamerule` 명령어 또는 바닐라 게임 규칙 편집 화면을 통해 인게임 [[GameRules 참조|ko_kr-26.1.2-GameRules]] 값을 직접 변경해야 합니다.
...
```
Cloth Config 설정 화면 ("Magnet, Let me get that! Configuration")
