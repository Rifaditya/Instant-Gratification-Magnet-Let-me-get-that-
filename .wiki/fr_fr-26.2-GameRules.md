# ⚙️ Référence Complète des GameRules (MC 26.2)
...
Toutes les mécaniques globales de **Magnet, Let me get that!** sont régies par des GameRules Minecraft avec espace de noms, regroupées sous la catégorie `magnet:magnet_category`.

Les administrateurs peuvent ajuster ces règles directement en jeu à l'aide de la commande `/gamerule <règle> <valeur>` ou via l'écran d'édition des règles de jeu lors de la création ou de la configuration d'un monde.
...
## ⚖️ Préséance des GameRules sur la Configuration JSON Globale

> ⚠️ **Avis Important pour les Administrateurs Serveur** :  
> Les paramètres définis dans le fichier de configuration JSON client/serveur (`config/ig_magnet.json`) ne fixent que les **valeurs par défaut initiales des nouveaux mondes créés**. Les mondes déjà existants sont régis exclusivement par leurs GameRules. Modifier `ig_magnet.json` n'altérera pas les paramètres d'une sauvegarde existante tant qu'ils n'auront pas été mis à jour via `/gamerule`.

---

## 🔗 Documentation Connexe
* [[Physique d'Attraction & Traversée de Phase|fr_fr-26.2-Vacuum-and-Phase-Shifting]]
* [[Interface de Configuration YACL & Valeurs par Défaut|fr_fr-26.2-Configuration]]
