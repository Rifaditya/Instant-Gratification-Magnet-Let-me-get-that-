...
├── MagnetCommand.java                  # 伺服端指令註冊 (/magnet toggle)
├── MagnetManager.java                  # 實體掃描與空間排程執行迴圈
├── MagnetMod.java                      # 模組進入點與網路包監聽
├── MagnetModClient.java                # 客戶端初始化、Ctrl+M 快捷鍵與覆蓋提示
├── MagnetMovement.java                 # 軌跡向量數學、動態插值速度與粒子
├── MagnetPlayerState.java              # 執行緒安全 ConcurrentHashMap 玩家開關存儲
├── MagnetTogglePayload.java            # 網路資料包 Record 與 StreamCodec 組合
├── SecondaryVisionCheck.java           # 細粒度方塊射線檢測 (植被、方塊實體、玻璃)
├── config/
│   ├── ClothConfigScreenHelper.java    # Cloth Config Fabric GUI 構建器
│   ├── MagnetConfig.java               # JSON 設定存儲與 POJO 欄位
│   └── ModMenuIntegration.java         # 反射安全的 ModMenu API 進入點
├── mixin/
│   ├── MixinEntity.java                # 向 Entity 注入 NoClip 穿相與重力抵消
│   └── PlayerMixin.java                # 向 Player 注入 Tick 執行與瞬間拾取
└── registry/
...
目標類別為 `net.minecraft.world.entity.player.Player`。
