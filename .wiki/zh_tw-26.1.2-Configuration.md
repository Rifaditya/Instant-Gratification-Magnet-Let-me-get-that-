...
| **GUI 函式庫** | Cloth Config Fabric (`me.shedaniel.cloth:cloth-config-fabric:26.1.154`) |
| **ModMenu 入口類別** | `net.instantgratification.magnet.config.ModMenuIntegration` |
| **GUI 畫面輔助類別** | `net.instantgratification.magnet.config.ClothConfigScreenHelper` |
...
在 Minecraft 26.1.2 中，**Magnet, Let me get that!** 整合了 **Cloth Config Fabric** 與 **ModMenu**，提供乾淨優雅的遊戲內視覺化設定畫面。
...
> 若要修改當前已經創建並正在運行的世界設定，請使用遊戲內 [[⚙️ 遊戲規則 GameRules 參考|zh_tw-26.1.2-GameRules]]（透過 `/gamerule` 或原版遊戲規則編輯畫面）。
