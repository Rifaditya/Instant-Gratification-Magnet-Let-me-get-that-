# ⚙️ Vollständige GameRules-Referenz (MC 26.2)
...
Alle globalen Mechaniken von **Magnet, Let me get that!** werden über namespaced Minecraft GameRules gesteuert, die unter der Kategorie `magnet:magnet_category` registriert sind.

Admins können diese Regeln im laufenden Spiel mit dem Befehl `/gamerule <regel> <wert>` oder über den regulären Bildschirm „Spielregeln bearbeiten“ beim Erstellen oder Bearbeiten einer Welt anpassen.
...
## ⚖️ Vorrang der GameRules vor globaler JSON-Konfiguration

> ⚠️ **Wichtiger Hinweis für Server-Administratoren**:  
> Einstellungen in der Client/Server-JSON-Konfigurationsdatei (`config/ig_magnet.json`) legen lediglich die **Basis-Standardwerte für NEU erstellte Welten** fest. Aktive Welten werden ausschließlich durch GameRules gesteuert. Das Ändern von `ig_magnet.json` wirkt sich nicht auf bestehende Spielstände aus, solange sie nicht via `/gamerule` aktualisiert werden.
...
* [[YACL-Konfigurations-GUI|de_de-26.2-Configuration]]
