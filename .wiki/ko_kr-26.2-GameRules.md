...
**Magnet, Let me get that!** 의 모든 글로벌 메커니즘은 `magnet:magnet_category` 헤더 아래 등록된 네임스페이스 마인크래프트 GameRules를 통해 제어됩니다.

서버 관리자는 인게임에서 `/gamerule <규칙> <값>` 명령어를 실행하거나, 월드 생성 및 편집 시 바닐라 "게임 규칙 편집" 화면을 통해 실시간으로 규칙을 설정할 수 있습니다.
...
## ⚖️ 글로벌 JSON 설정 대비 GameRules 우선순위

> ⚠️ **서버 관리자를 위한 중요 안내 사항**:  
> 클라이언트/서버 JSON 설정 파일(`config/ig_magnet.json`)의 설정은 **새로 생성되는 신규 월드의 기본값**만을 정의합니다. 활성화된 기존 월드는 오직 GameRules에 의해 제어됩니다. 이미 생성된 월드에서 설정을 변경하려면 인게임 `/gamerule` 명령어를 사용해야 합니다.
...
* [[설정 GUI 및 JSON 동기화|ko_kr-26.2-Configuration]]
