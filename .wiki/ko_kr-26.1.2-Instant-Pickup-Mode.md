...
| **대상 변수** | 플레이어 수집 바운딩 박스 (`AABB pickupArea`) |

---

## 📖 즉시 수집(Instant Pickup) 개요

마인크래프트 26.1.2에서 **즉시 수집 모드**는 `Player.aiStep()` 에서 플레이어의 수집 바운딩 박스를 확장하여, 드롭된 아이템을 바닐라 수집 핸들러를 통해 플레이어 인벤토리로 즉각 유입시킴으로써 비행 시간을 완전히 제거합니다.
...
| GameRule | 타입 | 기본값 | 설명 |
| :--- | :---: | :---: | :--- |
| `ig:magnet_instant` | Boolean | `false` | true 설정 시 아이템이 비행하지 않고 플레이어에게 즉시 순간이동됩니다. |
| `ig:magnet_range` | Integer | `12` | 확장할 수집 영역의 블록 반경입니다. |
| `ig:magnet_enabled` | Boolean | `true` | 모드 전체를 켜거나 끄는 마스터 스위치입니다. |
