...
├── IMagnetPlayer.java                  # 玩家接口，提供个人开关状态及 Getter/Setter
├── MagnetCommand.java                  # Brigadier 命令树 (/magnet toggle, /magnet debug)
├── MagnetDebugLogger.java              # 线程安全的持久化文件异步日志记录器
├── MagnetManager.java                  # 实体扫描与空间运算执行主循环
├── MagnetMod.java                      # 服务端/通用模组入口点及网络接收端
├── MagnetModClient.java                # 客户端模组初始化器、按键绑定与悬浮提示
...
│   └── YaclScreenHelper.java           # YetAnotherConfigLib v3 GUI 构建器
├── mixin/
│   ├── MixinEntity.java                # 向 Entity 注入 NoClip 状态与条件重力消除
│   └── PlayerMixin.java                # 向 Player 注入 NBT 持久化与瞬间拾取逻辑
├── registry/
│   └── ModGameRules.java               # DynamicGameRuleManager 游戏规则注册表
└── util/
    └── ModVersionGuard.java            # 运行时 Knot ClassLoader 安全完整性检查
```
...
目标类为 `net.minecraft.world.entity.player.Player` 并实现 `IMagnetPlayer` 接口。

| 注入方法名 | 注入挂钩切入点 | 行为动作与功能逻辑 |
| :--- | :--- | :--- |
| `ig_magnet$addAdditionalSaveData` | `@Inject(method = "addAdditionalSaveData", at = @At("TAIL"))` | 通过 `ValueOutput` 将布尔值 `ig_magnet_enabled` 序列化写入玩家 NBT。 |
| `ig_magnet$readAdditionalSaveData` | `@Inject(method = "readAdditionalSaveData", at = @At("TAIL"))` | 通过 `ValueInput.getBooleanOr()` 从玩家 NBT 反序列化读取磁铁状态。 |
| `ig_magnet$tick` | `@Inject(method = "tick", at = @At("HEAD"))` | 每个游戏刻在服务端调用 `MagnetManager.tick(player)` 执行吸附。 |
| `ig_magnet$expandPickupArea` | `@ModifyVariable(method = "aiStep", at = @At("STORE"), ordinal = 0)` | 当 `ig:magnet_instant` 为 true 时扩展拾取盒（`pickupArea.inflate(range)`）。 |

---

## 🛡️ Knot 类加载器完整性校验 (`ModVersionGuard`)

为防止服务器或世界存档在错误的、不兼容的 Minecraft 游戏版本上强行加载：
