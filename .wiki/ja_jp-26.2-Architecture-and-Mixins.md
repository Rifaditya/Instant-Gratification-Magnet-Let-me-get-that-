...
├── IMagnetPlayer.java                  # トグル状態およびアクセサ用プレイヤーインターフェース
├── MagnetCommand.java                  # Brigadier コマンドツリー (/magnet toggle, /magnet debug)
├── MagnetDebugLogger.java              # ディスクへのスレッドセーフな永続ファイルロガー
├── MagnetManager.java                  # エンティティスキャンおよび空間処理ループ
├── MagnetMod.java                      # サーバー/共通 MOD エントリポイントおよびパケット受信
├── MagnetModClient.java                # クライアント MOD 初期化、キーバインドおよび通知表示
...
│   └── YaclScreenHelper.java           # YetAnotherConfigLib v3 GUI ビルダー
├── mixin/
│   ├── MixinEntity.java                # Entity への NoClip および重力キャンセル注入
│   └── PlayerMixin.java                # Player への NBT 永続化および即時回収処理の注入
├── registry/
│   └── ModGameRules.java               # DynamicGameRuleManager によるゲームルール登録
└── util/
    └── ModVersionGuard.java            # ランタイム Knot ClassLoader 整合性チェック
```
...
| `ig_magnet$conditionalCancelGravity` | `@Inject(method = "applyGravity", at = @At("HEAD"), cancellable = true)` | アイテムが固体のブロック内部にある場合*のみ*下方向の重力加速度をキャンセル。 |

---

### 2. `net.instantgratification.magnet.mixin.PlayerMixin`
`net.minecraft.world.entity.player.Player` を対象とし、`IMagnetPlayer` を実装します。

| 注入メソッド | 注入ターゲットポイント | 動作および挙動 |
| :--- | :--- | :--- |
| `ig_magnet$addAdditionalSaveData` | `@Inject(method = "addAdditionalSaveData", at = @At("TAIL"))` | `ValueOutput` を介してプレイヤー NBT にブーリアン `ig_magnet_enabled` を保存。 |
| `ig_magnet$readAdditionalSaveData` | `@Inject(method = "readAdditionalSaveData", at = @At("TAIL"))` | `ValueInput.getBooleanOr()` を介してプレイヤー NBT から `ig_magnet_enabled` を復元。 |
...
## 🛡️ Knot ClassLoader 整合性チェック (`ModVersionGuard`)

破損したバージョン、未リリース版、または互換性のない Minecraft バージョンでのワールド読み込みを防ぐためのガードクラスです：

```java
public final class ModVersionGuard {
    public static void checkClass(String modName, String requiredClassName) {
        ClassLoader contextLoader = Thread.currentThread().getContextClassLoader();
        // MOD の初期化を続行する前にターゲットクラスの存在を検証...
    }
}
```

`MagnetMod.onInitialize()` 内での呼び出し:
```java
